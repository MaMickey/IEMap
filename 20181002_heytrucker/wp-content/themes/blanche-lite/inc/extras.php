...
function blanche_lite_notice() {
	global $current_user;
	$user_id = $current_user->ID;
	if (!get_user_meta($user_id, 'blanche_notice_ignore')) {
		echo '<div class="updated notice"><p>'. esc_html__('Thanks for installing Blanche Lite! Want more features?', 'blanche-lite') .' <a href="https://www.vivathemes.com/wordpress-theme/blanche/" target="blank">'. esc_html__('Check Out the Pro Version  &#8594;', 'blanche-lite') .'</a><a class="notice-dismiss" href="?blanche-ignore-notice"><span class="screen-reader-text">Dismiss Notice</span></a></p></div>';
	}
}
add_action('admin_notices', 'blanche_lite_notice');

function blanche_notice_ignore() {
	global $current_user;
	$user_id = $current_user->ID;
	if (isset($_GET['blanche-ignore-notice'])) {
		add_user_meta($user_id, 'blanche_notice_ignore', 'true', true);
	}
}
add_action('admin_init', 'blanche_notice_ignore');

add_action('admin_head', 'blanche_admin_style');
function blanche_admin_style() {
  echo '<style>
