...
class SR_Helper
{
	...
	/**
	 * @param $occupancy_child_age_range
	 *
	 * @return string
	 */
	public static function render_list_occupancy_child_age_range($occupancy_child_age_range = null)
	{
		$options = '';
		for ($j = 0; $j <= 10; $j++)
		{
			$selected = '';
			if (isset($occupancy_child_age_range) && $occupancy_child_age_range == $j)
			{
				$selected = 'selected';
			}
	...
	/**
	 * Get extra group for room type edit screen
	 *
	 * @param int $reservation_asset_id
	 * @param int $id
	 *
	...
	public static function render_extras_group($reservation_asset_id = 0, $id = 0)
	{
		global $wpdb;
		$get_page                      = $_GET['page'];
		$check_reservation_exist_extra = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}sr_extras WHERE reservation_asset_id = %d AND charge_type IN (0,4,5,6)", $reservation_asset_id));
		$extra_groupbox                = '';
		if ($get_page == 'add_new_room_type')
		{
		...
		else
		{
			...
			{
				$get_extras         = $wpdb->get_results($wpdb->prepare("SELECT id, name FROM {$wpdb->prefix}sr_extras WHERE reservation_asset_id = %d AND charge_type IN (0,4,5,6)", $reservation_asset_id));
