...
class SR_Reservations_Table_Data extends Solidres_List_Table {
	...
	function __construct() {
		...
		$query_default  = "SELECT r.* FROM {$wpdb->prefix}sr_reservations as r";
		...
		$filter_customer_fullname    = isset( $_GET['filter_customer_fullname'] ) ? esc_sql($_GET['filter_customer_fullname']) : '';
		$filter_guest_fullname       = isset( $_GET['filter_guest_fullname'] ) ? esc_sql($_GET['filter_guest_fullname']) : '';
		...
		if ( $filter_guest_fullname != '' ) {
			$query_default .= ' INNER JOIN ' . $wpdb->prefix . 'sr_reservation_room_xref AS rr ON rr.reservation_id = r.id AND rr.guest_fullname LIKE "%' . $filter_guest_fullname . '%"';
		}

		if ( is_numeric( $filter_published ) ) {
			$query_filter[] = ' r.state = ' . $filter_published;
		}
		if ( $filter_reservation_asset_id > 0 ) {
			$query_filter[] = ' r.reservation_asset_id = ' . $filter_reservation_asset_id;
		}
		if ( is_numeric( $filter_payment_status ) ) {
			$query_filter[] = ' r.payment_status = ' . $filter_payment_status;
		}
		if ( $filter_customer_fullname != '' ) {
			$query_filter[] = ' r.customer_firstname LIKE "%' . $filter_customer_fullname . '%" OR r.customer_middlename LIKE "%' . $filter_customer_fullname . '%" OR r.customer_lastname LIKE "%' . $filter_customer_fullname . '%"';
		}

		if ( $filter_checkin_from != '' && $filter_checkin_to != '' ) {
			$query_filter[] = ' r.checkin >= "' . date( 'Y-m-d', strtotime( $filter_checkin_from ) ) . '" AND r.checkin <= "' . date( 'Y-m-d', strtotime( $filter_checkin_to ) ) . '"';
		}
		if ( $filter_checkout_from != '' && $filter_checkout_to != '' ) {
			$query_filter[] = ' r.checkout >= "' . date( 'Y-m-d', strtotime( $filter_checkout_from ) ) . '" AND r.checkout <= "' . date( 'Y-m-d', strtotime( $filter_checkout_to ) ) . '"';
		}
		if ( ( $string_search != '' && is_null( $filter_reservation ) ) || ( $string_search != '' && isset( $filter_reservation ) && $filter_reservation_asset_id == '' && $filter_published == '' && $filter_payment_status == '' && $filter_customer_fullname == '' && $filter_checkin_from == '' && $filter_checkin_to == '' && $filter_checkout_from == '' && $filter_checkout_to == '' ) ) {
			if ( stripos( $string_search, 'id:' ) === 0 ) {
				$query_default = $query_default . ' WHERE r.id = ' . (int) substr( $string_search, 3 );
			} else {
				$query_default = $query_default . ' WHERE r.code LIKE "%' . $string_search . '%"';
			}
			...
			$query_default = $query_default . " WHERE r.state IN (0,1,2,3,4,5)";
		} else if ( $state != null && is_null( $filter_reservation ) ) {
			$query_default = $query_default . " WHERE r.state = " . $state;
			...
			if ( $string_search == '' ) {
				...
				$query_default = $query_default . ' WHERE ' . implode( ' AND', $query_filter ) . ' AND r.code LIKE "%' . $string_search . '%"';
			}
		}

		$query_default = $query_default . ' ORDER BY r.id DESC';
		...
		foreach ( $results as $result ) {
			...
			$this->items[] = array(
				'codename'      => '<span class="' . $badges[ $result->state ] .' reservation-code"><strong><a class="row-title" href="?page=' . $_REQUEST['page'] . '&action=edit&id=' . $result->id . '" aria-label="'. $result->code .'">'. $result->code . '</a></strong></span>',
	...
	function extra_tablenav( $which ) {
		...
		<div class="alignleft actions bulkactions">
            ...
            <input type="text" name="filter_guest_fullname"
                   value="<?php if ( isset( $_GET['filter_guest_fullname'] ) ) {
				       echo $_GET['filter_guest_fullname'];
			       } ?>" placeholder="<?php _e( 'Search by guest name', 'solidres' ); ?>"/>
