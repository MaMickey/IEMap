		...
		<div class="reservation_note_group">
			<?php foreach ( $get_reservation_notes as $reservation_note ) {
				...
				<div class="reservation_note_item">
					<p class="info"><?php echo $reservation_note->created_date; ?> <?php _e(' by ', 'solidres' ) ?> <?php echo $created_name; ?></p>
					<?php if ( ! current_user_can( 'solidres_user' ) ) { ?>
						<p><?php _e( 'Notify customer via email:', 'solidres' ); ?> <?php echo $reservation_note->notify_customer == 1 ? __( 'Yes', 'solidres' ) : __( 'No', 'solidres' ); ?>
							| <?php _e( 'Display in frontend:', 'solidres' ); ?> <?php echo $reservation_note->visible_in_frontend == 1 ? __( 'Yes', 'solidres' ) : __( 'No', 'solidres' ); ?></p>
					<?php } ?>
