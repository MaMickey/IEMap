...
function sr_edit_reservation_item( $id ) {
	...
	<script>
		jQuery(function($) {
            ...
            $.fn.editable.defaults.emptytext = '<?php _e( 'Empty', 'solidres') ?>';
			$("#state").editable({
				source: [
			...
			$("#payment_status").editable({
				source: [
					{value: 0, text: '<?php _e('Unpaid', 'solidres' ) ?>'},
					{value: 1, text: '<?php _e('Completed', 'solidres' ) ?>'},
					{value: 2, text: '<?php _e('Cancelled', 'solidres' ) ?>'},
					{value: 3, text: '<?php _e('Pending', 'solidres' ) ?>'}
				],
				params: function (params) {
					params.action = 'solidres_edit_reservation_field';
					params.security = '<?php echo wp_create_nonce( 'edit-reservation' ) ?>';
	...
	<div class="wrap">
		<div id="wpbody">
			...
			<h2>
				<?php printf( esc_html__( 'Edit reservation %s', 'solidres' ), $sr_form_data->code ); ?>
				<a
