...
class SR_Ajax {
	...
	public static function load_coupons() {
		...
		foreach ( $coupons as $coupon ) {
			$coupons_data .= '<input type="checkbox" name="srform[coupons][]" value="'.$coupon->id.'"/><a href="' . admin_url( "admin.php?page=sr-coupons&action=edit&id=" . $coupon->id ) . '" target="_blank">'.apply_filters( 'solidres_coupon_name', $coupon->coupon_name ).'</a><br>';
		}
	...
	public static function load_extras() {
		...
		$extras_data = '';
		$reservation_asset_id = (int)$_REQUEST['reservation_asset_id'];
		$extras = $wpdb->get_results( $wpdb->prepare( "SELECT id, name FROM {$wpdb->prefix}sr_extras WHERE reservation_asset_id = %d AND charge_type IN (0,4,5,6)", $reservation_asset_id ) );
		foreach ( $extras as $extra ) {
			$extras_data .= '<input type="checkbox" name="srform[extras][]" value="'.$extra->id.'"/><a href="' . admin_url( "admin.php?page=sr-extras&action=edit&id=" . $extra->id ) . '" target="_blank">'.apply_filters( 'solidres_extra_name', $extra->name ).'</a><br>';
		}
	...
	public static function save_reservation_note() {
		...
		foreach ( $get_reservation_notes as $get_reservation_note ) {
			...
			$notify_customer =  $get_reservation_note->notify_customer == 1 ? __( 'Yes', 'solidres' ) : __( 'No', 'solidres' );
			$visible_frontend = $get_reservation_note->visible_in_frontend == 1 ? __( 'Yes', 'solidres' ) : __( 'No', 'solidres' );

			$notes_data .= '<div class="reservation_note_item">';
			$notes_data .= '<p class="info">'.$get_reservation_note->created_date.' by '.$created_name.'</p>';
			$notes_data .= '<p>'. __('Notify customer via email:', 'solidres' ) . ' ' . $notify_customer.' | ' . __( 'Display in frontend:', 'solidres' ) . ' '.$visible_frontend.'</p>';
	...
	public static function load_available_rooms() {
		...
		solidres()->session->set( 'sr_code', $reservation->code );
		...
		if ( !empty($room_types) ) {
			foreach ( $room_types as &$room_type ) {
				...
				if (!defined('SR_PLUGIN_COMPLEXTARIFF_ENABLED') || !SR_PLUGIN_COMPLEXTARIFF_ENABLED)  {
					...
					$complex_tariffs = $solidres_tariff->load_by_room_type_id( $room_type->id, false, OBJECT, $checkin, $checkout, 1, -1 );
				...
				if (!empty($checkin) && !empty($checkout)) {
					...
					if (defined( 'SR_PLUGIN_DISCOUNT_ENABLED' ) && SR_PLUGIN_DISCOUNT_ENABLED)
					{
						...
						$discounts = $solidres_discount->load_discounts( $checkin, $checkout, array(1), 1, $asset_id);
					}
				...
				if ( !empty( $rooms ) ) {
					...
					foreach ( $rooms as $room ) {
						$isAvailable = $solidres_reservation->is_room_available( $room->id, $checkin, $checkout, $asset->booking_type );
	...
	public static function load_roomtype()
	{
		check_ajax_referer( 'load-roomtype', 'security' );
		...
		if ($room_type_id > 0) {
			$room_type = $wpdb->get_row($wpdb->prepare( "SELECT * FROM {$wpdb->prefix}sr_room_types WHERE id = %d", $room_type_id));
			echo json_encode( $room_type );
		}
		wp_die();
	}

	public static function load_roomtypes()
	{
		check_ajax_referer( 'load-roomtypes', 'security' );

		$asset_id = (int) $_REQUEST['id'];

		if ($asset_id > 0) {
			$solidres_roomtype = new SR_Room_Type();
			$room_types = $solidres_roomtype->load_by_asset_id($asset_id);
			$results = array();
